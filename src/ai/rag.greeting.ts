  ...
  async generatePersonalizedGreeting(
    ...
    try {
      ...
      if (this.deps.redisClient) {
        const greetingKey = `rag:ward:${wardId}:greeting`;
  ...
  private async generateGreetingWithLLM(
    ...
    try {
      ...
      if (
        ...
        this.deps.logger.error(
          `Invalid response structure: ${JSON.stringify(responseBody)}`,
        );
        throw new Error('Invalid LLM response structure');
      }

      if (responseBody.content.length === 0) {
        this.deps.logger.error('Empty content array in response');
        throw new Error('Empty content in LLM response');
      }

      // Extract greeting with null check
      const firstContent = responseBody.content[0];
      if (!firstContent || typeof firstContent.text !== 'string') {
        this.deps.logger.error(
          `Invalid content format: ${JSON.stringify(firstContent)}`,
        );
      ...
      if (!greeting || greeting.length === 0) {
        this.deps.logger.error('Empty greeting text after trim');
        throw new Error('Empty greeting generated by LLM');
      }
